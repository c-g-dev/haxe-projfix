...
class FixPackages {
    ...
    public static function run(args:Array<String>) {
        if (args.length == 0 || args[0] == "-h" || args[0] == "--help") {
            Sys.println("Usage:");
            Sys.println("  haxe --run FixPackages path/to/project.hxml");
            Sys.println("Options:");
        ...
        for (root in cpSet.keys()) {
            ...
            for (f in hxFiles) {
               ...
               var expected = expectedPackage(root, f);
               var res = fixFile(f, expected, dryRun);
               filesProcessed++;
               if (res.changed) {
                  filesChanged++;
    ...
    static function walk(dir:String, out:Array<String>):Void {
        ...
        for (name in entries) {
            ...
            var path = dir + "/" + name;
            var isDir = false;
            try isDir = FileSystem.isDirectory(path) catch (_:Dynamic) continue;
            if (isDir) {
                walk(path, out);
            } else {
    ...
    static function expectedPackage(root:String, filePath:String):String {
        var r = normSlash(FileSystem.fullPath(root));
        var f = normSlash(FileSystem.fullPath(filePath));
        if (!StringTools.endsWith(r, "/")) r += "/";

        var rel = StringTools.startsWith(f, r) ? f.substr(r.length) : f;
        var dir = Path.directory(rel);
        if (dir == null || dir == "" || dir == "." || dir == "/") return "";
        dir = dir.split("\\").join("/");
        if (StringTools.startsWith(dir, "./")) dir = dir.substr(2);
        while (StringTools.endsWith(dir, "/")) dir = dir.substr(0, dir.length - 1);
        if (dir == "") return "";
        var parts = [for (p in dir.split("/")) if (p != "" && p != ".") p];
        return parts.join(".");
    }
    ...
    static function fixFile(filePath:String, expected:String, dryRun:Bool):{ changed:Bool, message:String } {
        ...
        if (expected == "") {
            if (info.has) {
                var before = content.substring(0, info.start);
                var after = content.substring(info.end);
                if (before.length > 0 && after.length > 0) {
                    var lastBefore = before.charAt(before.length - 1);
                    var firstAfter = after.charAt(0);
                    if ((lastBefore == "\n" || lastBefore == "\r") && (firstAfter == "\n" || firstAfter == "\r")) {
                        after = after.substr(1);
                    }
                }
                if (!dryRun) File.saveContent(filePath, before + after);
                changed = true;
                message = "removed package";
            }
        } else {
            if (info.has) {
                var current = info.name == null ? "" : info.name;
                if (current != expected) {
                    var replacement = "package " + expected + ";";
                    var newContent = content.substring(0, info.start) + replacement + content.substring(info.end);
                    if (!dryRun) File.saveContent(filePath, newContent);
                    changed = true;
                    message = (current == "" ? "set package to " + expected : ("changed package " + current + " -> " + expected));
                }
            } else {
                var insertPos = info.headerEnd;
                var statement = "package " + expected + ";" + nl;
                var newContent = content.substring(0, insertPos) + statement + content.substring(insertPos);
                if (!dryRun) File.saveContent(filePath, newContent);
                changed = true;
                message = "added package " + expected;
            }
        }
